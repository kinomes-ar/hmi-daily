...
.topnav a[aria-current="page"]{border-bottom-color:var(--red)}
.topnav .badge{
  display:inline-block; min-width:16px; padding:1px 5px; margin-left:4px; border-radius:999px;
  background:var(--red); color:var(--onred); font-size:9px; text-align:center; line-height:1.4;
}
.topnav .badge:empty{display:none}

/* ---- hearts ---- */
.acts{display:flex; align-items:center; gap:14px; margin-top:9px}
.acts .src{margin-top:0}
.col.side .acts{justify-content:center}
@media (max-width:660px){ .col.side .acts{justify-content:flex-start} }
html[data-filter] .col.side .acts{justify-content:flex-start}
.heart{
...
.heart svg{width:15px; height:15px; display:block; fill:none; stroke:currentColor; stroke-width:1.8; stroke-linejoin:round}
...
.grid .ph{aspect-ratio:3/2}
.grid .hl{font-size:19px; line-height:1.18}
.grid .tx{font-size:12.5px; line-height:1.5; color:var(--muted)}
.grid .tagrow{margin-left:0; margin-right:0}
.empty{padding:60px 0 40px; text-align:center; color:var(--muted); font-size:14px; line-height:1.7}
.empty .tx{margin:0 auto}
.empty .big{font-family:"Archivo Black",Impact,sans-serif; font-size:clamp(24px,4vw,40px); color:var(--ink); text-transform:uppercase; line-height:1; margin-bottom:14px}
.clearbtn{
  appearance:none; cursor:pointer; background:transparent; border:1px solid var(--ink); border-radius:999px;
  color:var(--ink); font:inherit; font-size:9.5px; font-weight:700; letter-spacing:.12em; text-transform:uppercase;
  padding:5px 12px; line-height:1.4;
}
.clearbtn:hover{background:var(--shade)}

/* ---- weekly ---- */
.week{padding:6px 0 12px}
.week + .week{padding-top:34px; border-top:2px solid var(--ink)}
.wtitle{
  font-family:"Noto Serif Display","Times New Roman",Georgia,"Songti SC","Apple SD Gothic Neo",serif;
  font-weight:550; font-stretch:80%; font-size:clamp(26px,3.6vw,40px); line-height:1.08; letter-spacing:-.01em;
...
.wcat li{display:flex; align-items:baseline; gap:10px; font-size:13px; line-height:1.35}
.wcat li a{color:var(--ink); text-decoration:none; border-bottom:1px solid var(--hair)}
.wcat li a:hover{color:var(--red); border-bottom-color:var(--red)}
.wcat li .who{font-size:9.5px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:var(--muted); white-space:nowrap}
.wtop{margin-top:30px}
.wtop h3{
  font-family:"Archivo Black",Impact,sans-serif; font-size:13px; letter-spacing:.06em; text-transform:uppercase;
  margin:0 0 4px; display:flex; align-items:center; gap:8px;
}
.wtop h3 svg{width:15px; height:15px; fill:var(--red); stroke:var(--red); stroke-width:1.8}
.wtop ol{list-style:none; margin:0; padding:0; counter-reset:n; display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:0 28px}
.wtop li{counter-increment:n; display:flex; gap:12px; align-items:baseline; padding:12px 0; border-bottom:1px solid var(--hair)}
.wtop li::before{content:counter(n,decimal-leading-zero); font-family:"Archivo Black",Impact,sans-serif; font-size:18px; color:var(--red); line-height:1}
...
def site_cfg():
    try:
        return json.load(open(os.path.join(HERE, "site.json")))
    except Exception:
        return {}
...
def slug(tag):
    return "".join(ch for ch in (tag or "").lower() if ch.isalpha())
...
def article(it, kind, photo_on=True, date="", iid=""):
    ...
    return (
        '<article class="a-{k}" data-tag="{slug}" data-id="{id}">{ph}'
...
def load_days():
    ...
    for f in files:
        ...
        days.append((d, json.load(open(f))))
    return days


def items_index(days):
    """Flat index of every story — the My page and Weekly page render from this."""
    out = []
    for d, items in days:
        for i, it in enumerate(items):
            out.append({
                "id": item_id(d, i), "date": d, "tag": it.get("tag", "News"),
                "t": it["t"], "en": it.get("en", ""), "zh": it.get("zh", ""), "ko": it.get("ko", ""),
                "src": it["src"], "url": it["url"], "img": it.get("img", ""),
            })
    return out
...
def render_days(days):
    ...
    for d, items in days:
        ...
        lead = article(items[0], "lead", date=d, iid=item_id(d, 0)) if items else ""
        duo = ""
        if len(items) > 2:
            duo = ('<div class="duo">%s%s</div>' %
                   (article(items[1], "duo", date=d, iid=item_id(d, 1)),
                    article(items[2], "duotx", photo_on=False, date=d, iid=item_id(d, 2))))
        mid = "".join(article(it, "mid", date=d, iid=item_id(d, i)) for i, it in enumerate(items) if 3 <= i < 5)
        side = "".join(article(it, "side", date=d, iid=item_id(d, i)) for i, it in enumerate(items) if 5 <= i < 8)
        foot = ""
        if items[8:]:
            foot = '<div class="rest">%s</div>' % "".join(
                article(it, "foot", date=d, iid=item_id(d, i)) for i, it in enumerate(items) if i >= 8)
        band = ('<div class="band">'
...
def load_weeks():
    files = sorted(glob.glob(os.path.join(WEEKLY, "*.json")), reverse=True)
...
def render_weeks(weeks, index, counts):
    ...
    for w in weeks:
        ...
        if not top:
            ...
            ranked = sorted(((counts.get(i, 0), i) for i in wk_ids), reverse=True)
            top = [{"id": i, "n": n} for n, i in ranked[:5] if n > 0]
        tops = []
        for t in top:
            it = byid.get(t["id"]) if isinstance(t, dict) else byid.get(t)
            if not it:
                continue
            n = t.get("n", counts.get(it["id"], 0)) if isinstance(t, dict) else counts.get(it["id"], 0)
    ...
    if not out:
        out.append('<div class="empty"><div class="big">Coming Monday</div>'
                   ...
                   '<p class="tx zh">首期周报将于下周一 10:00 发布。</p>'
                   '<p class="tx ko">첫 위클리 요약은 다음 주 월요일 10:00에 나옵니다.</p></div>')
    return "".join(out)
...
<div class="shell">
  <div class="top">
    ...
    <nav class="topnav" aria-label="Sections">
      <a href="index.html"{c_daily}>Daily</a>
      <a href="weekly.html"{c_weekly}>Weekly</a>
      <a href="my.html"{c_my}>My &#9829;<span class="badge" id="mybadge"></span></a>
    </nav>
    <span class="ctrls">
...
(function(){
  ...
  function cid(){
    try{
      ...
      if(!c){ c=Math.random().toString(36).slice(2,10)+Date.now().toString(36); localStorage.setItem('adux-cid',c); }
      return c;
    }catch(e){ return 'anon'; }
  }
  function post(path,body){
    if(!API||!window.fetch) return Promise.reject(new Error('no api'));
...
FILTERS = """  <div class="fwrap"><nav class="filters" aria-label="Category filter">
...
(function(){
  ...
  function card(it){
    ...
    var ph=it.img?'<figure class="ph ph-foot"><img src="'+esc(it.img)+'" alt="" loading="lazy"></figure>'
                 :'<figure class="ph ph-foot fb" style="background:'+c+';color:#fff"><span>'+esc(it.tag)+'</span></figure>';
    var tx=['en','zh','ko'].map(function(k){ return it[k]?'<p class="tx '+k+'">'+esc(it[k])+'</p>':''; }).join('');
    return '<article class="a-foot" data-tag="'+s+'" data-id="'+esc(it.id)+'">'+ph+
  ...
  function empty(){
    if(!items&&Object.keys(window.ADUX.likes()).length){
      return '<div class="empty"><div class="big">Archive index unavailable</div>'+
        '<p class="tx en">Your hearts are safe on this device — reload to try again.</p>'+
        '<p class="tx zh">你的收藏仍保存在本设备上——请刷新重试。</p>'+
        '<p class="tx ko">하트는 이 기기에 그대로 저장돼 있어요. 새로고침해서 다시 시도해 주세요.</p></div>';
    }
    return '<div class="empty"><div class="big">Nothing here yet</div>'+
      '<p class="tx en">Tap the heart on any story and it will be kept here — on this device, no login needed.</p>'+
      '<p class="tx zh">点一下任意文章的心形，它就会保存在这里——仅在本设备上，无需登录。</p>'+
      '<p class="tx ko">기사 옆 하트를 누르면 여기에 모입니다. 이 기기에만 저장되고 로그인은 필요 없어요.</p></div>';
  }
  var items=null;
  function draw(){
    var likes=window.ADUX.likes();
    var ids=Object.keys(likes).sort(function(a,b){ return likes[b]-likes[a]; });
    var clr=document.getElementById('clearall'); if(clr) clr.hidden=!ids.length;
    if(!items||!ids.length){ host.innerHTML=empty(); host.className=''; return; }
    var by={}; items.forEach(function(it){ by[it.id]=it; });
    var html=ids.map(function(id){ return by[id]?card(by[id]):''; }).join('');
...
def standalone(frag, lang="en"):
    return ('<!doctype html><html lang="%s"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width,initial-scale=1">' % lang
            + frag.replace("<style>", "</head><body><style>", 1) + "</body></html>")


def refresh_likes():
    """Pull the shared heart counts into likes.json (runner only — needs network)."""
    api = site_cfg().get("like_api", "")
    if not api:
...
if __name__ == "__main__":
    args = sys.argv[1:]
    # the runner refreshes the shared-count snapshot on every rebuild
    if "--likes" in args or os.environ.get("GITHUB_ACTIONS"):
        refresh_likes()
